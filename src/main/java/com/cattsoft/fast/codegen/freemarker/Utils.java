...
public class Utils {
	...
	public static List<HashMap<String, Object>> getColumnDataType(String tableName, int mysqlOrOracle)
			throws SQLException {
		OperateDB operateDb = new OperateDB();
		...
		if (mysqlOrOracle == MYSQL) {
			...
			strSql = new StringBuilder(
					"select c.COLUMN_NAME,c.DATA_TYPE,c.DATA_LENGTH,c.DATA_PRECISION,c.DATA_SCALE,cm.COMMENTS from USER_TAB_COLUMNS c, USER_COL_COMMENTS cm ");
			strSql.append(" where c.TABLE_NAME = cm.TABLE_NAME and c.COLUMN_NAME=cm.COLUMN_NAME  and  c.TABLE_NAME='"
					+ tableName).append("'");
		}
